    ...
    BaseFrame(parent, id, title, pos, size, style)
{
    ...
    if (d)
    {
        std::vector<wxString> charsets;
        d->fillVector(charsets, wxT("select rdb$character_set_name from rdb$character_sets order by 1"));
        for (std::vector<wxString>::iterator it = charsets.begin(); it != charsets.end(); ++it)
        {
            if ((*it) == wxT("NONE"))
                continue;
            ch_charset->Append(*it);
        }
    }
...
{
    loadCollations(ch_collate->GetStringSelection());
}
//-----------------------------------------------------------------------------
void FieldPropertiesFrame::loadCollations(wxString desired)
{
    Database* d = tableM->getDatabase();    // get list of collations
    if (!d)
        return;

    std::vector<wxString> list = d->getCollations(ch_charset->GetStringSelection());
    int to_select = 0;
    int counter = 0;
    ch_collate->Clear();
    for (std::vector<wxString>::iterator it = list.begin(); it != list.end(); ++it)
    {
        ch_collate->Append(*it);
        if ((*it) == desired)
            to_select = counter;
        counter++;
    }
...
void FieldPropertiesFrame::OnChDomainsClick(wxCommandEvent& WXUNUSED(event))
{
    ...
    if (domain != wxT("[new]"))
        updateDomainInfo(ch_domains->GetStringSelection());
    updateEditBoxes();
...
void FieldPropertiesFrame::OnButtonOkClick(wxCommandEvent& WXUNUSED(event))
{
    ...
    {
        if (fieldM->getName() != fieldName)     // field name changed
            sql += wxT("ALTER TABLE ") + tableM->getName() + wxT(" ALTER ")
                + fieldM->getName() + wxT(" TO ") + fieldName + wxT(";\n\n");
        ...
        // changed domain
        if (fieldM->getSource() != selectedDomain && selectedDomain != wxT("[new]"))
        {
            sql += wxT("ALTER TABLE ") + tableM->getName() + wxT(" ALTER ") + fieldName +
                wxT(" TYPE ") + selectedDomain + wxT(";\n\n");
        }
        else if (selectedDomain == wxT("[new]")     // user domain -> autogenerated domain
            || type != selectedDatatype     // OR changed datatype, size or scale
            || size != tsize
            || scale != tscale)
        {
            sql += wxT("ALTER TABLE ") + tableM->getName() + wxT(" ALTER ") + fieldName +
                wxT(" TYPE ");
            sql += selectedDatatype;
            if (!tsize.IsEmpty())
            {
                sql += wxT("(") + tsize;
                if (!tscale.IsEmpty())
        ...
        if (cb_notnull->IsChecked() == fieldM->isNullable())    // watch for double negation!
        {
            ...
            sql += wxT(" \nWHERE RDB$FIELD_NAME = '") + fieldName
                + wxT("' AND RDB$RELATION_NAME = '") + tableM->getName() + wxT("';\n\n");
        }
    }
    else    // new field
    {
        sql += wxT("ALTER TABLE ") + tableM->getName() + wxT(" ADD \n") + fieldName + wxT(" ");
    ...
    if (update_not_null != unnNone)
    {
        ...
        wxString sqladd = wxT("UPDATE ") + tableM->getName() + wxT(" \nSET ")
            + fieldName + wxT(" = '") + s + wxT("' \nWHERE ")
            + fieldName + wxT(" IS NULL;\n");
    ...
    sql += textctrl_sql->GetValue();  // execute autoinc sql (gen+trigger),
...
void FieldPropertiesFrame::setProperties()
{
    ...
    wxString genname = wxT("GEN_") + tableM->getName() + wxT("_ID");
    textctrl_generatorname->SetValue(genname);
    ...
    for (MetadataCollection<Generator>::const_iterator it = d->generatorsBegin(); it != d->generatorsEnd(); ++it)
        ch_generators->Append((*it).getName());
    ch_domains->Clear();
    if (fieldM == 0 || fieldM->getSource().substr(0,4) != wxT("RDB$"))
    {
    ...
    for (MetadataCollection<Domain>::const_iterator it = d->domainsBegin(); it != d->domainsEnd(); ++it)
    {
        ...
        if (name.substr(0, 4) == wxT("RDB$") && (!fieldM || fieldM->getSource() != name))
            continue;
        ch_domains->Append(name);
    }

    if (fieldM)
    {
        // editing existing field...
        textctrl_fieldname->SetValue(fieldM->getName());
        cb_notnull->SetValue(!fieldM->isNullable());
        ch_domains->SetSelection(ch_domains->FindString(fieldM->getSource()));
...
//-----------------------------------------------------------------------------
bool FieldPropertiesFrame::getDomainInfo(wxString domain, wxString& type, wxString& size,
    wxString& scale, wxString& charset)
{
...
//-----------------------------------------------------------------------------
void FieldPropertiesFrame::updateDomainInfo(wxString domain)
{
    wxString type, size, scale, charset;
    ...
    ch_datatypes->SetSelection(ch_datatypes->FindString(type));
    ch_charset->SetSelection(ch_charset->FindString(charset));
}
//-----------------------------------------------------------------------------
...
void FieldPropertiesFrame::updateSqlWindow()
{
    ...
    wxString sql;
    wxString generator = ch_generators->GetStringSelection();
    if (radio_new->GetValue())
    {
        generator = textctrl_generatorname->GetValue();
...
//-----------------------------------------------------------------------------
const wxString FieldPropertiesFrame::getName() const
{
    return wxT("FieldPropertiesFrame");
}
//-----------------------------------------------------------------------------
...
{
    if (uri.action != wxT("edit_field") && uri.action != wxT("add_field"))
        return false;
    ...
    FieldPropertiesFrame* f = new FieldPropertiesFrame(w, -1,
